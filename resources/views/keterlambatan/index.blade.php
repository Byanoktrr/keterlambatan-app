...
@section('content')
    ...
    <div id="kt_header" style="" class="header align-items-stretch">
        ...
        <div class="toolbar">
            <div
                ...
                <div class="page-title d-flex flex-column me-5">
                    <h1 class="d-flex flex-column text-dark fw-bolder fs-3 mb-0">Data Keterlambatan</h1>
        ...
        <div class="my-4">
            ...
            <a href="{{ route('keterlambatan.export-excel') }}" class="btn btn-info text-white">Export Data Keterlambatan</i></a>
        </div>
        ...
        <div class="tab-content pt-5" id="tab-content">
            <div class="tab-pane active" id="simple-tabpanel-0" role="tabpanel" aria-labelledby="simple-tab-0">
                <div class="d-flex justify-content-between">
                    <form action="{{ route('keterlambatan.index') }}" method="get"  class="d-flex">
                                            ...
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h1 class="modal-title fs-5" id="exampleModalLabel">Konfigurasi
                                                            Hapus
                                                        </h1>
                ...
                {{ $lates->appends(['search' => $search ?? ''])->links('pagination::bootstrap-4') }}
            </div>
                    ...
                    <tbody>
                        ...
                        @foreach ($rekap as $late)
                            <tr>
                                ...
                                <td>{{ $late->students->name }}</td>
                                <td>{{ $late->total }}</td>
                                <td><a href="{{ route('keterlambatan.show', $late['student_id']) }}"
                                        class="btn btn-primary me-2">show</a>
                                    @if ($late->total > 2)
                                        <a href="{{ route('keterlambatan.print', ['student_id' => $late->student_id]) }}"
                                            class="btn btn-primary me-2">Print Surat Pernyataan</a>
                                    @endif
                                </td>
                                <td class="d-flex"> </td>
                            </tr>
    ...
    @else
    <div class="my-4">
        <a href="{{ route('keterlambatan.export-excelPs') }}" class="btn btn-info text-white">Export Data Keterlambatan</i></a>
    </div>
    <ul class="nav nav-tabs" role="tablist">
        <li class="nav-item" role="presentation">
            <a class="nav-link active" id="simple-tab-0" data-bs-toggle="tab" href="#simple-tabpanel-0" role="tab"
    ...
    <div class="tab-content pt-5" id="tab-content">
        <div class="tab-pane active" id="simple-tabpanel-0" role="tabpanel" aria-labelledby="simple-tab-0">
            <div class="d-flex justify-content-between">
                <form action="{{ route('keterlambatan.indexPs') }}" method="get"  class="d-flex">
                    <div class="input-group">
                        <input type="text" name="search" class="form-control" placeholder="Cari siswa..."
                            value="{{ isset($search) ? $search : '' }}">
                        <button type="submit" class="btn btn-primary">Cari</button>
            ...
            <table class="table table-bordered table-striped mt-3">
                ...
                <tbody>
                    @php
                        $no = 1;
                    @endphp
                   @foreach ($studentgroup as $late)
                   @php
                       $countlate = $late->where('student_id', $late['student_id'])->count();
                   @endphp
                        <tr>
                            <td>{{ $no++ }}</td>
                            <td>{{ $late->students->nis }}</td>
                            <td>{{ $late->students->name }}</td>
                            <td>{{ $countlate }}</td>
                            <td><a href="{{ route('keterlambatan.showPs', $late['student_id']) }}"
                                    class="btn btn-primary me-2">show</a>
                                @if ($countlate > 2)
                                    <a href="{{ route('keterlambatan.printPs', ['student_id' => $late->student_id]) }}"
                                        class="btn btn-primary me-2">Print Surat Pernyataan</a>
                                @endif
                            </td>
                            <td class="d-flex"> </td>
